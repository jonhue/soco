use crate::{algorithms::online::multi_dimensional::lazy_budgeting::smoothed_load_optimization::{
        lb, Memory, Options,
    ...
    }, problem::IntegralSmoothedLoadOptimization, streaming::online::{self, OfflineResponse}};
...
fn start(
    ...
    w: i32,
    options: Options,
) -> PyResult<SLOResponse<i32, Memory>> {
    py.allow_threads(|| {
        let OfflineResponse {
            xs: (xs, cost),
...
) -> PyResult<SLOStepResponse<i32, Memory>> {
    py.allow_threads(|| {
