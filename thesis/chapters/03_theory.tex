...
\paragraph{Continuity and Differentiability} A very first natural restriction is to assume that hitting costs are continuous and \emph{smooth}\index{smooth} where we call a function smooth if it is infinitely-many times continuously differentiable. However, it is immediate that the operating costs in our data center model are not smooth. At best we can guarantee piecewise smoothness. Thus it seems like the assumption of smoothness is too strong to be interesting for the application of right-sizing data centers.
