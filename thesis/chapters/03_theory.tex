...
\begin{definition}\index{approximation algorithm}
...
\begin{definition}\index{competitive algorithm}
...
This definition is also known as \textit{dynamic regret}\index{dynamic regret} \cite{Chen2018}. We next define a variant of regret that bridges between static and dynamic regret.
...
We now observe that given the optimal offline solution $OPT$ with schedule $x_t^*$, the $L$-constrained dynamic regret is equivalent to dynamic regret for $L = \sum_{t=1}^T \norm{x_t^* - x_{t-1}^*}$. The literature also considers other metrics such as the \textit{$\alpha$-unfair competitive ratio}\index{$\alpha$-unfair competitive ratio} which are very similar to $L$-constrained dynamic regret \cite{Andrew2015}.
...
\begin{problem}[Knapsack (KP)]\index{knapsack}
