    ...
    return Box::new(|&(t, i): &Vertice| {
        if t == p.t_end {
            ...
            vec![0; p.m as usize]
                .iter()
                .enumerate()
                .map(|(j, _)| ((t + 1, j as i32), build_cost(&p, t, i, j as i32)))
                .collect()
        }
    });
}
...
fn build_cost(&p: &DiscreteProblem, t: i32, i: i32, j: i32) -> Cost {
    return OrderedFloat(
        p.beta * discrete_pos(i - j) + (p.f)(t, i).expect("f should be total on its domain"),
    );
}

fn select_initial_neighbors(&p: &DiscreteProblem, neighbors: Neighbors) -> Neighbors {
...
fn find_schedule(&p: &DiscreteProblem, neighbors: Neighbors) -> DiscreteSchedule {
